...
> **Upgrade auf 0.12.0 — betrifft nur englischsprachige Instanzen.** Vier Sensoren heißen dort jetzt so, wie sie es auf Deutsch seit 0.7.0 tun: `rain` ist „Station precipitation today" (gemessen), `tmax_today`, `tmin_today` und `rain_today` sind „Forecast rest of day …" (Prognose). Vorher klangen alle vier nach Messwerten. **Die Entity-IDs bleiben unverändert** — nur der angezeigte Name ändert sich, Dashboards und Automationen laufen weiter. Eine englische Neuinstallation bekommt allerdings IDs nach dem neuen Namen; wer beides angleichen will, benennt die Entität in Home Assistant selbst um.
