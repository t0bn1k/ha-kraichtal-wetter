...
- **Stundenvorhersage**: Die Einträge in `hours` tragen nur eine Beschriftung („Jetzt", „15:00"), kein Datum. `_hourly_datetimes()` verankert die Reihe an der ersten beschrifteten Stunde und zählt in UTC weiter (Zeitumstellung), `API_TIME_ZONE` ist fest `Europe/Berlin` — die API rechnet in dieser Zone, unabhängig von der Zeitzone der Instanz. Herleitung in `docs/API.md`.
  - **Zeitumstellung, vom Betreiber bestätigt (17.09.2026):** 24 feste Slots pro Kalendertag. Im Oktober gibt es **genau ein** `"02:00"` (zwei Instanzen werden gemittelt), im März fehlt `"02:00"` ganz. Die Reihe ist in diesen zwei Nächten nicht garantiert lückenlos; der Code fängt das ab, weil bei einem Widerspruch die Beschriftung gilt. **Nicht neu herleiten** — nachgerechnet, dokumentiert, erledigt.
  - **Ein Feld `time` je Eintrag wird bewusst nicht genutzt** (verworfen 24.09.2026). Der Betreiber hat es angekündigt, aber die Rückrechnung ist bestätigt und reicht; `time` würde nur in den zwei Umstellungsnächten etwas ändern. Auch `current.obs_time` ersetzt es nicht — das ist die Uhrzeit der letzten *Messung*, nicht der Vorhersagestunden. Begründung in `Plan.md`, Punkt 9.
...
- **Key-Prüfung in den Flows**: Einrichtung und Reauth rufen `_async_validate_key()` (`config_flow.py`) auf und zeigen `invalid_auth` / `cannot_connect` / `unknown` am Formular, statt einen falschen Key zu speichern. Sie nutzt `async_get_data()`, nicht `async_update()` — Letzteres loggt „starting reauth", was während der Eingabe falsch wäre.
- **Logging bei Ausfällen**: Der Client loggt Abruffehler nur auf `debug`. Der `DataUpdateCoordinator` meldet das `UpdateFailed` selbst — einmal beim Ausfall, einmal bei der Erholung. Ein eigenes `error` hier stünde bei jedem Abruf im Log. Ausnahme: die Warnung vor dem Reauth bei 401/403.
